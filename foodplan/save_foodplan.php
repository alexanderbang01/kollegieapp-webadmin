...
try {
    ...
    if ($check_result->num_rows == 0) {
        // Opret en ny madplan med standardværdier
        $stmt = $conn->prepare("INSERT INTO foodplan (week_number, year, monday_dish, tuesday_dish, wednesday_dish, thursday_dish, thursday_description, monday_vegetarian, tuesday_vegetarian, wednesday_vegetarian, thursday_vegetarian) VALUES (?, ?, '', '', '', 'Det lune bord', 'Varme rester fra ugens retter. En blanding af de bedste smagsvarianter fra mandag til onsdag.', 0, 0, 0, 0)");
    ...
    // Opdater alle dage inkl. torsdag (nu kan torsdag også redigeres)
    ...
    foreach ($days as $day) {
        ...
        if (isset($_POST[$dish_key])) {
            // Denne dag blev sendt i formularen - opdater alle dens felter
            $dish = $_POST[$dish_key];
            $description = isset($_POST[$day . '_description']) ? $_POST[$day . '_description'] : '';
            $vegetarian = isset($_POST[$day . '_vegetarian']) ? 1 : 0;

            // Hvis torsdag er tom, brug standardværdier
            if ($day === 'thursday') {
                if (empty(trim($dish))) {
                    $dish = 'Det lune bord';
                }
...
// Redirect tilbage til madplan siden (uden edit mode)
