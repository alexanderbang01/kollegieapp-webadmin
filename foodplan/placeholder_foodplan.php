...
try {
    ...
    // Standard madplan data (opdateret med fast torsdag)
    $placeholderMeals = [
        'monday' => [
        ...
        'thursday' => [
            ...
            'vegetarian' => 0,
            'allergens' => [] // Ingen faste allergener for rester
        ]
    ];
    ...
    if ($check_result->num_rows == 0) {
        ...
        $stmt->bind_param(
            ...
            $week_number,
            $year,
            $placeholderMeals['monday']['dish'],
            $placeholderMeals['monday']['description'],
            $placeholderMeals['monday']['vegetarian'],
            $placeholderMeals['tuesday']['dish'],
            $placeholderMeals['tuesday']['description'],
            $placeholderMeals['tuesday']['vegetarian'],
            $placeholderMeals['wednesday']['dish'],
            $placeholderMeals['wednesday']['description'],
            $placeholderMeals['wednesday']['vegetarian'],
            $placeholderMeals['thursday']['dish'],
            $placeholderMeals['thursday']['description'],
            $placeholderMeals['thursday']['vegetarian']
        );
    ...
    // Tilføj allergener for hver dag (kun mandag-onsdag, torsdag har ingen faste allergener)
    ...
    foreach ($placeholderMeals as $day => $meal) {
        if ($day !== 'thursday' && isset($meal['allergens']) && is_array($meal['allergens'])) {
...
// Redirect tilbage til madplan siden (behold edit mode)
header("Location: ./index.php?week={$week_number}&year={$year}&edit=true");
exit;
