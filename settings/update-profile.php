...
try {
    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
        ...
        $filename_only = 'profile_' . $user_id . '_' . time() . '.' . $file_extension;
        ...
        // Gem fuld URL i stedet for bare filnavn
        $profile_image_filename = 'http://localhost/kollegieapp-webadmin/employees/images/' . $filename_only;
        ...
        if (!empty($current_user['profile_image'])) {
            // Tjek om det er en fuld URL eller bare et filnavn
            if (strpos($current_user['profile_image'], 'http') === 0) {
                // Det er en fuld URL - konverter til filsystem sti
                $old_filename = basename($current_user['profile_image']);
                $old_image_path = $upload_dir . $old_filename;
            } else {
                // Det er bare et filnavn (gammelt format)
    ...
    // Opdater session profilbillede hvis nyt billede blev uploadet
    if ($profile_image_filename) {
