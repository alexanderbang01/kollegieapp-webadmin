...
// Tjek om event_id er angivet
$event_id = isset($_GET['id']) ? (int)$_GET['id'] : null;

if (!$event_id) {
    $_SESSION['error_message'] = "Intet event-ID angivet";
    header("Location: ./");
    exit();
}

// Hent begivenheden fra databasen
$event = null;
if (isset($conn)) {
    ...
    if ($result->num_rows > 0) {
        ...
        $_SESSION['error_message'] = "Begivenheden blev ikke fundet";
        header("Location: ./");
        exit();
    }
}

// Tjek om brugeren har ret til at redigere begivenheden
...
if ($user_role == 'Administrator') {
    $can_edit = true;
} elseif ($event['created_by'] == $user_id) {
    $can_edit = true;
}

if (!$can_edit) {
    $_SESSION['error_message'] = "Du har ikke tilladelse til at redigere denne begivenhed";
    header("Location: ./");
    exit();
}
?>
        ...
        <main class="flex-grow">
            ...
            <div class="p-3 sm:p-6">
                <div class="mb-4 sm:mb-6 flex justify-between items-center">
                    <div>
                        <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Rediger begivenhed</h1>
                        <p class="text-sm sm:text-base text-gray-600">Ændre detaljer for denne begivenhed</p>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow p-4 sm:p-6 animate-fade-in">
                    <form id="edit-event-form" action="save-event.php" method="POST" class="space-y-6">
                        <!-- Skjulte felter til event ID og user ID -->
                        <input type="hidden" name="event_id" value="<?php echo $event['id']; ?>">
                        <input type="hidden" name="created_by" value="<?php echo $event['created_by']; ?>">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="md:col-span-2">
                                    ...
                                    <textarea id="description" name="description" rows="4" required
                                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary/50"><?php echo htmlspecialchars($event['description']); ?></textarea>
                                </div>
                                <div>
